...
{
    // The sketch's Framework must be initialized before creating a ShaderProgram.
    if(!framework.initialized) {
        throw "Initialize framework before creating a ShaderProgram.";
    }
    ...
    var fragmentShader = this.getShader(framework.shaderProgramData[id].shaders[0]);

    // Get the ShaderProgram's vertex shader.
    var vertexShader = this.getShader(framework.shaderProgramData[id].shaders[1]);
    ...
    if(!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
        throw "Could not initialize ShaderProgram " + this.id;
    }
    ...
    this.drawType = framework.shaderProgramData[this.id].drawType;

    // Get attribute and uniform pointers.
    this.attributes = []; // Array of attribute pointers.
    this.uniforms = [];   // Array of uniform pointers.
    for(var i=0; i<framework.shaderProgramData[this.id].attributes.length; i++)
    {
        var att = framework.shaderProgramData[this.id].attributes[i];
    ...
    if(framework.shaderProgramData[this.id].uniforms.length > 0)
    {
        // If so, get pointers to their locations and store in this.uniforms.
        for (i = 0; i < framework.shaderProgramData[this.id].uniforms.length; i++)
        {
            var uni = framework.shaderProgramData[this.id].uniforms[i];
            var glvar = framework.uniforms[uni].glvar;
...
ShaderProgram.prototype.getShader = function(shaderName) {
    ...
    $shader = $shader.replace('HEATOFFSET', maxHeat.toFixed(8));
    $shader = $shader.replace('HEATSCALE', (1 / (2 * maxHeat)).toFixed(8));
    $shader = $shader.replace('HEATRESCALE', (2 * maxHeat).toFixed(8));
...
ShaderProgram.prototype.prep = function(reloadAttributes) {
    ...
    if(this.uniforms.length > 0) {
        var unames = framework.shaderProgramData[this.id].uniforms;
        for(var i=0; i<unames.length; i++) {
            var uname = unames[i];
            var data = framework.uniforms[uname].data;
            var type = framework.uniforms[uname].type;
            if (type === gl.FLOAT) {
